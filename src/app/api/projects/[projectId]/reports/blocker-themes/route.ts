...
import { fetchBlockerThemes } from "@/lib/reports/data";
...
export async function GET(
  ...
  try {
    ...
    logError("Failed to authorize blocker themes report", error);
    return NextResponse.json(
  ...
  try {
    const data = await fetchBlockerThemes(filters);
    return NextResponse.json({ ok: true, data });
  } catch (error) {
    logError("Failed to load blocker themes report", error);
    return NextResponse.json(
      { ok: false, message: "Failed to load blocker themes report" },
      { status: 500 }
    );
  }
}
