...
import { fetchVelocityPoints } from "@/lib/reports/data";
...
export async function GET(
  ...
  try {
    ...
    logError("Failed to authorize velocity report", error);
    return NextResponse.json(
  ...
  try {
    const data = await fetchVelocityPoints(filters);
    return NextResponse.json({ ok: true, data });
  } catch (error) {
    logError("Failed to load velocity report", error);
    return NextResponse.json(
      { ok: false, message: "Failed to load velocity report" },
      { status: 500 }
    );
  }
}
