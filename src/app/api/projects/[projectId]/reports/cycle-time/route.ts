...
import { fetchCycleTimeReport } from "@/lib/reports/data";
...
export async function GET(
  ...
  try {
    ...
    logError("Failed to authorize cycle time report", error);
    return NextResponse.json(
  ...
  try {
    const data = await fetchCycleTimeReport(filters);
    return NextResponse.json({ ok: true, data });
  } catch (error) {
    logError("Failed to load cycle time report", error);
    return NextResponse.json(
      { ok: false, message: "Failed to load cycle time report" },
      { status: 500 }
    );
  }
}
